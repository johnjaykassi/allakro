@extends('admin.layout.app', ['title' => 'Liste des personnes qui ont deménagé'])

@section('content')
    <div class="main-panel grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">Les personnes qui ont deménagé ({{ $deme->count() }})</h4>
                    ...
                    <tbody>
                        @foreach ($deme as $item)
                            <tr>
                                ...
                                <td>{{ $item->nou_destination }}</td>
                                <td>{{ $item->city_live }}</td>
                                <td> {{ $item->created_at }} </td>
                                <td>
                                    @if ($item->status == 1)
                                        <span title="En ligne"class="bg-success">
                                            <form action="{{ route('active.demenage', $item->id) }}" method="post">
                                                @csrf
                                                {{ method_field('PUT') }}
